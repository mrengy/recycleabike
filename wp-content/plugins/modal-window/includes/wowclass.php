        ...
        if ($count > 0) {
			...
			if (count($result) > 0) {
				foreach ($result as $key => $val) {
					...
					$path_script = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] ."/asset/wowscript-".$lastid.".js";
					ob_start();
					include ($file_script);
				...
				if (file_exists ( $file_style )){
					$path_style = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] ."/asset/wowstyle-".$lastid.".css";
					ob_start();
					include ($file_style);
    ...
    function updItem($tblname, $wowinfo) {
        ...
        if ($count > 0) {
			...
			foreach ($result as $key => $val) {
				$file_script = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] .'/admin/js/script.php';
				if (file_exists ( $file_script )){
					$path_script = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] ."/asset/wowscript-".$wowid.".js";
					ob_start();
					include ($file_script);
					$content_script = ob_get_contents();
					$packer = new JavaScriptPacker($content_script, 'Normal', true, false);
				...
				$file_style = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] .'/admin/css/style.php';
				if (file_exists ( $file_style )){
					$path_style = WP_PLUGIN_DIR.'/'.$wowinfo["plugdir"] ."/asset/wowstyle-".$wowid.".css";
					ob_start();
					include ($file_style);
					$content_style = ob_get_contents();										
					ob_end_clean();
